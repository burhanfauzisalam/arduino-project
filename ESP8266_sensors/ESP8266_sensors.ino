...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // Kontrol berdasarkan nilai STRESS atau RELAX
  else if (strcmp(topic, "hope/data") == 0) {
    if (strcmp(message, "STRESS") == 0) {
      digitalWrite(LED1_PIN, HIGH);
      digitalWrite(LED2_PIN, HIGH);
      digitalWrite(LED3_PIN, HIGH);
      Serial.println("Semua lampu dinyalakan (STRESS)");
    } else if (strcmp(message, "RELAX") == 0) {
      digitalWrite(LED1_PIN, HIGH);
      digitalWrite(LED2_PIN, LOW);
      digitalWrite(LED3_PIN, LOW);
      Serial.println("Hanya Lampu 1 yang dinyalakan (RELAX)");
    }
  }
}
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("ESP8266Client")) {
      ...
      // Subscribe ke topik masing-masing lampu dan hope/data
      client.subscribe("light1/status");
      client.subscribe("light2/status");
      client.subscribe("light3/status");
      client.subscribe("hope/data");
      
      Serial.println("Subscribed ke topik: light1/status, light2/status, light3/status, hope/data");
    } else {
